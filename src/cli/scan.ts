...
import { discoverFlacFiles } from '../flac/discovery.js';
...
export const scanCommand = defineCommand({
	...
	async run({ args }) {
		try {
			...
			if (config.fix) {
				await ensureBinary('id3v2', 'brew install id3v2');
			}
			...
			try {
				...
				logScanComplete(config.log_path, {
					...
					total: verificationStats
						? verificationStats.healthy + verificationStats.corrupt + verificationStats.id3Fixed + verificationStats.pruned
						: 0,
				});
